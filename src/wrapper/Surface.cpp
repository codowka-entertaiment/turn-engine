...
bool surface::convert(TurnEngine::pixel_format const& fmt) noexcept {
    return SDL_ConvertSurface(surface_, fmt.native_handle(), 0) != nullptr ? true : false;
}

surface surface::convert_to_new(TurnEngine::pixel_format const& fmt) const noexcept {
...
TurnEngine::blend_mode surface::blend_mode() const noexcept {
    SDL_BlendMode mode{};
    [[maybe_unused]] auto const err = SDL_GetSurfaceBlendMode(surface_, &mode);
    SDL2_ASSERT(err == 0);
    return static_cast<TurnEngine::blend_mode>(mode);
}
...
bool surface::set_blend_mode(TurnEngine::blend_mode const mode) noexcept {
